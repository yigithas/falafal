...
myForm.addEventListener('submit', async function(event){
    ...
    if (!yapayZekaModeli) {
        alert("Yapay Zeka henüz yüklenmedi, lütfen birkaç saniye bekleyin.");
        return;
    }

    if (falResmiInput.files.length === 0) {
    ...
    const tahminler = await yapayZekaModeli.classify(resimOnizleme);
    
    const kahveKelimeleri = ['cup', 'mug', 'saucer', 'coffee', 'espresso', 'tableware'];
    let fincanBulunduMu = false;

    tahminler.forEach(tahmin => {
        if (tahmin.probability > 0.15) {
...
async function jsondanRastgeleVeriCek() {
    try {
        ...
        const ask_yorumlar = tumYorumlar.filter(satir => {
        return satir.yas == yas_enum && satir.meslek == meslek_enum && satir.tarz==1 ;
        });
        const rastgeleIndeks1 = Math.floor(Math.random() * ask_yorumlar.length);
        const ask_yorumu = ask_yorumlar[rastgeleIndeks1];

        const is_yorumlar = tumYorumlar.filter(satir => {
        return satir.yas == yas_enum && satir.meslek == meslek_enum && satir.tarz==2 ;
        });
        const rastgeleIndeks2 = Math.floor(Math.random() * is_yorumlar.length);
        const is_yorumu = is_yorumlar[rastgeleIndeks2];

        const aile_yorumlar = tumYorumlar.filter(satir => {
        return satir.yas == yas_enum && satir.meslek == meslek_enum && satir.tarz==3 ;
        });
        const rastgeleIndeks3 = Math.floor(Math.random() * aile_yorumlar.length);
        const aile_yorumu = aile_yorumlar[rastgeleIndeks3];

        const gelecek_yorumlar = tumYorumlar.filter(satir => {
        return satir.yas == yas_enum && satir.meslek == meslek_enum && satir.tarz==4 ;
        });
        ...
        const para_yorumlar = tumYorumlar.filter(satir => {
        return satir.yas == yas_enum && satir.meslek == meslek_enum && satir.tarz==5 ;
        });
